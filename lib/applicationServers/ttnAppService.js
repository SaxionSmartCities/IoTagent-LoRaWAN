...
class TtnAppService extends appService.AbstractAppService {
    ...
    preProcessMessage (mqttTopic, message) {
        ...
        if (splittedMqttTopic.length !== 4) {
            ...
            if (message && (message['payload_fields'] || message['payload_raw'])) {
                this.messageHandler(this, deviceId, deviceId, message['payload_fields'] || message['payload_raw']);
            } else {
                this.messageHandler(this, deviceId, deviceId, null);
            }
        }
    }
